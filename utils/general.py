...
def mask_large_box(bboxes: torch.Tensor, shape, min_size=32):
    ...
    Args:
        ...
        shape: [ [h0, w0],  [[h/h0, w/w0], [pad_h, pad_w]] ]
            where `h0` and `w0` is the height and width of original images (e.g., [1080, 1920]),
            `h` and `w` is the image size chosen in the option (e.g., [360, 640]), and
            `pad_h` and `pad_w` is the size of padding (e.g., [12, 16], then the transformed image size
            is [360 + 2 * 12, 640 + 2 * 16] = [384, 672]).
        min_size: the size in original image. After rescaling (w, h) in bboxes to the original size,
            we filter out them if one of rescaled w or h is smaller than `min_size`.

    Return:
        torch.Tensor of torch.bool type.
    """
    # Assume that every images have same shapes
    (h0, w0), ((ratio_h_h0, ratio_w_w0), (pad_h, pad_w)) = shape
    h, w = h0 * ratio_h_h0, w0 * ratio_w_w0
    max_leng = max(h + 2 * pad_h, w + 2 * pad_w)
    h_thres, w_thres = min_size / h0 * h / max_leng, min_size / w0 * w / max_leng

    mask_w = bboxes[:, 0] >= w_thres
    mask_h = bboxes[:, 1] >= h_thres
...
def ap_per_class(
    ...
    if plot:
        ...
        if wlog:
            ...
            wandb.log(
                {f"{test_type}/PR_curve": [wandb.Image(fig2img(fig), caption="Label")]},
                commit=False,
            )
...
def non_max_suppression(
    ...
    Returns:
        List[torch.Tensor]: list of filtered predictions for each image.
            Each element is [n, 6] shape of (x1, y1, x2, y2, conf, cls) rows.
    """
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # TensorRT holds multiplied value
        if trt:
            x[:, 5] = x[:, 4]

        # Box (center x, center y, width, height) to (x1, y1, x2, y2)
        # box = xywh2xyxy(x[:, :4])
        box = x[:, :4]  # Detect has been changed to emit xyxy coordinates.
...
def get_wlog_box_data(box_info, xywh=False, normal=False):
    ...
    Args:
        box_info (np.ndarray): each row is [cls_id, x1, y1, x2, y2(, conf, iou)]
        normal (bool): if True, use `percentage` coordinate for distance in image.
        xywh (bool): if True, [cls_id, x, y, w, h, ...]
    ...
    if len(box_info) == 0 or len(box_info[0]) not in [5, 7]:
        ...
        return box_data
    # Ground Truth label have no confidence, IoU columns
    ...
    for row in box_info:
        ...
        if gt_label:
            ...
            box_datum["scores"] = {"confidence": 1.0, "IoU": 1.0}
        else:
            cls_id, x1, y1, x2, y2, conf, iou = row
            box_datum["scores"] = {"confidence": float(conf), "IoU": float(iou)}
            box_datum[
                "box_caption"
            ] = f"Conf_{float(conf * 100):.2f}:IoU_{float(iou * 100):.2f}"
        # Using np.float type occurs "json serialization error"
        if xywh:
            x, y, w, h = x1, y1, x2, y2
            box_datum["position"] = {
        ...
        box_datum["domain"] = "percentage" if normal else "pixel"
        box_datum["class_id"] = int(cls_id)
        box_data.append(box_datum)
    return box_data
...
class BBoxScore:
    ...
    def _check_container(self):
        ...
        for fp, bboxes in self.fp2bboxes.items():
            ...
            for test_type, bbox_data in self.fp2bboxes[fp].items():
                assert isinstance(bbox_data, np.ndarray)

                if "gt" in test_type:
                    # (cls_id, x1, y1, x2, y2)
                    assert bbox_data.shape[1] == 5
                else:
                    # (cls_id, x1, y1, x2, y2, conf, best_iou)
                    assert bbox_data.shape[1] == 7

                # use Normal coordinates
                assert -eps < bbox_data[:, 1:5].all() < 1 + eps
    ...
    def get_filtered_fps(
        ...
        Args:
            criterion (str): filtering criterion. possible values are 'large_conf_small_iou', and 'small_conf_large_iou'.
            test_type (str): the test type on which the filtering is applied.
        """
        self._check_container()
        unchoosed = list(self.fp2bboxes.keys())
        max_n_fps = min(len(unchoosed), max_n_fps)
        ...
        if criterion:
            ...
            for fp, bboxes in self.fp2bboxes.items():
                ...
                conf_minus_best_iou = bbox_data[-2] - bbox_data[-1]
                # Any better way for this?
                fp2scores[fp] = {
                    "large_conf_small_iou": conf_minus_best_iou.max(),
                    "small_conf_large_iou": (-1 * conf_minus_best_iou).max(),
                }
    ...
    def get_wlog_img(self, fp, shift_cls_id=False):
        ...
        Args:
            ...
            shift_cls_id (bool): if True, shift class id for each test type.
        """
        if shift_cls_id:
            test_type2class_id_to_label = {}
            for test_type in self.test_types:
                test_type2class_id_to_label[test_type] = {
                    cls_id: label + "_" + test_type[0]
                    for cls_id, label in self.class_id_to_label.items()
                }
        img = self.fp2img_tsr[fp]
        boxes = self.fp2bboxes[fp]

        wdbboxes = {}
        for test_type, box_info in boxes.items():
            box = box_info.copy()
            if shift_cls_id:
                n_cls = len(self.class_id_to_label.keys())
                shift_n = n_cls * self.test_types2idx[test_type]
                if len(box) > 0:
                    box[:, 0] += shift_n
            xywh = False
            if "xywh" in test_type:
                xywh = True
            if len(box_info) == 0:
                continue
            box_data = get_wlog_box_data(box, xywh, normal=True)
            wdbboxes[test_type] = {
                "box_data": box_data,
                "class_lables": test_type2class_id_to_label[test_type]
                if shift_cls_id
                else self.class_id_to_label,
            }
        img_wdb = wandb.Image(img, boxes=wdbboxes)
